...
- **Legal Pages & About CMS**: Privacy Policy (`/privacy-policy`), Terms of Service (`/terms-of-service`), and No Refund Policy (`/refund-policy`) pages with comprehensive Indian law-compliant content (IT Act 2000, Consumer Protection Act 2019, Chennai jurisdiction). Content stored in `legal_pages` DB table; Super Admin can edit via Settings page. About page (`/about`) reads company story, vision, mission, founded year, and location from `company_settings` table (falls back to hardcoded defaults). Footer links updated with all 3 legal pages. Seed data auto-populates on first run.
...
- **Self-Service Password Reset**: Email-based "Forgot Password" flow — employees can request a reset link via email and set a new password. Reset tokens are hashed (SHA-256) at rest and expire after 1 hour. Atomic token consumption prevents replay attacks. SMTP settings are configurable by Super Admin in Settings page with connection-test button. Public endpoint checks SMTP status; forgot-password form shows clear message if SMTP not configured.
...
- **Legal Pages**: Privacy Policy, Terms of Service, No Refund Policy — stored in `legal_pages` table with slug, title, content, effectiveDate, updatedBy, updatedAt
